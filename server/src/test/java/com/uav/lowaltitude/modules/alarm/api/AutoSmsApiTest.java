...
class AutoSmsApiTest {
    ...
    @Test void failureIsDurableRetryOnlyQueuesAndUsesSameProviderKey()throws Exception {
        doReturn(new com.uav.lowaltitude.modules.alarm.application.AdvisorySmsPort.Delivery(true,"FAILED")).when(sms).simulate(anyString(),anyString(),anyString(),anyString());
    ...
    @Test void transportExceptionDoesNotProveSendingFailed() throws Exception {
        doThrow(new IllegalStateException("transport outcome unknown")).when(sms).simulate(anyString(),anyString(),anyString(),anyString());
        automatic.process(eventId);
        read().andExpect(jsonPath("$.data.auto_sms.status").value("UNKNOWN"))
                .andExpect(jsonPath("$.data.auto_sms.can_retry").value(false));
    }
    ...
    @Test void retryRequiresActionsScopeAndValidVersion()throws Exception {
        doReturn(new com.uav.lowaltitude.modules.alarm.application.AdvisorySmsPort.Delivery(true,"FAILED")).when(sms).simulate(anyString(),anyString(),anyString(),anyString());automatic.process(eventId);
