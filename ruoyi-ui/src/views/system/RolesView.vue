...
const menuLabels = { devices: '设备管理', monitor: '设备实时监测', commission: '设备接入调测', maps: '地图管理', users: '用户管理', roles: '角色管理', archive: '审计日志' }
...
function menuLabel(item) { return menuLabels[item.route_key] || item.name || item.permission_code }
...
<template>
  <div class="page-stack">
    ...
    <section v-loading="loading" class="content-card roles-layout">
      ...
      <main v-if="detail" class="role-detail">
        ...
        <el-tabs v-model="activePermissionTab" class="permission-tabs">
          <el-tab-pane name="menu">
            ...
            <el-table :data="menuPermissions" border empty-text="暂无菜单权限目录">
              <el-table-column label="菜单入口" min-width="220"><template #default="{row}"><el-checkbox :model-value="row.menu_enabled" :disabled="isPermissionLocked(row)" @change="value=>setMenu(row,value)">{{ menuLabel(row) }}</el-checkbox><small class="code-note">{{ row.permission_code }}</small></template></el-table-column>
              <el-table-column label="权限等级" width="170"><template #default="{row}"><span v-if="locked">全部</span><el-select v-else :model-value="row.level" :disabled="isPermissionLocked(row)" @change="value=>setLevel(row,value)"><el-option v-for="level in levels" :key="level.value" :label="level.label" :value="level.value" /></el-select></template></el-table-column>
              <el-table-column label="限制" width="150"><template #default="{row}">{{ locked ? '固定权限' : protectedCodes.has(row.permission_code) ? '仅超级管理员' : '可配置' }}</template></el-table-column>
            </el-table>
          </el-tab-pane>
    ...
    <el-dialog v-model="createDialog.visible" title="新增自定义角色" width="860px" destroy-on-close>
      ...
      <el-table :data="createDialog.permissions.filter(item=>item.route_key)" border height="360"><el-table-column label="菜单入口" min-width="230"><template #default="{row}"><el-checkbox v-model="row.menu_enabled" :disabled="protectedCodes.has(row.permission_code)" @change="value=>setMenu(row,value)">{{ menuLabel(row) }}</el-checkbox><small class="code-note">{{ row.permission_code }}</small></template></el-table-column><el-table-column label="权限等级" width="190"><template #default="{row}"><el-select v-model="row.level" :disabled="protectedCodes.has(row.permission_code)" @change="value=>setLevel(row,value)"><el-option v-for="level in levels" :key="level.value" :label="level.label" :value="level.value" /></el-select></template></el-table-column><el-table-column label="限制" width="150"><template #default="{row}">{{ protectedCodes.has(row.permission_code)?'仅超级管理员':'可配置' }}</template></el-table-column></el-table>
...
.role-detail{min-width:0;padding:20px;overflow:auto}.role-summary,.save-bar,.permission-intro{display:flex;align-items:center;justify-content:space-between;gap:16px}.role-summary{align-items:flex-start;padding-bottom:18px;border-bottom:1px solid var(--admin-border)}.role-summary h2,.role-summary p,.permission-intro h3,.permission-intro p{margin:0}.role-summary h2{color:var(--admin-text);font-size:20px;line-height:1.4}.role-summary p{margin-top:6px;color:var(--admin-muted);line-height:1.6}.role-summary__actions,.save-actions{display:flex;flex:none;align-items:center;gap:8px}
...
.code-note{display:block;margin-top:3px;color:var(--admin-muted);font-family:ui-monospace,monospace;font-size:11px}.action-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px}.action-group{padding:14px;border:1px solid var(--admin-border);border-radius:8px;background:#fbfcfe}.action-group h4{margin:0 0 8px;padding-bottom:10px;border-bottom:1px solid var(--admin-border);color:var(--admin-text)}.action-row{display:flex;min-height:40px;align-items:center;justify-content:space-between;gap:10px;padding:4px 0}.action-row>span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.action-row .el-select{width:104px}.save-bar{position:sticky;z-index:2;bottom:-20px;margin:20px -20px -20px;padding:14px 20px;border-top:1px solid var(--admin-border);background:rgba(255,255,255,.96);box-shadow:0 -6px 16px rgba(23,32,51,.04)}.save-bar>span{margin-right:auto;color:var(--admin-muted)}.role-fields{display:grid;grid-template-columns:1fr 1fr;gap:18px}
@media(max-width:1100px){.roles-layout{grid-template-columns:1fr}.role-sidebar{border-right:0;border-bottom:1px solid var(--admin-border)}.role-list{max-height:272px}.role-summary,.save-bar{align-items:flex-start;flex-direction:column}.save-actions{width:100%;justify-content:flex-end}.role-fields{grid-template-columns:1fr}}
@media(max-width:560px){.role-detail{padding:16px}.role-summary__actions,.save-actions{width:100%;flex-wrap:wrap}.permission-tabs :deep(.el-tabs__item){padding:0 14px}.action-grid{grid-template-columns:1fr}.save-bar{bottom:-16px;margin:16px -16px -16px;padding:12px 16px}}
</style>
