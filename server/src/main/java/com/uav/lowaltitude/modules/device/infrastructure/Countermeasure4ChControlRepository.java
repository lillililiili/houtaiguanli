...
@Repository
public class Countermeasure4ChControlRepository {
    private final JdbcTemplate jdbc;
    public Countermeasure4ChControlRepository(JdbcTemplate jdbc) { this.jdbc = jdbc; }

    public void insert(String commandId, String commandNo, String deviceId, String requestedBy, String reason,
                       String sourceMode, boolean simulated, long deadline, long now, String action, String channel,
                       Integer mask, String authorizationId) {
        jdbc.update("""
                ...
                VALUES (?,?,?,?,'COUNTERMEASURE_4CH',?,'QUEUED',?,?,?,?,?,?)
                """, commandId, commandNo, deviceId, requestedBy, reason, sourceMode, simulated, deadline, now, now,
                authorizationId);
        jdbc.update("""
                INSERT INTO countermeasure_4ch_command(command_id,action,channel,mask,authorization_id)
                VALUES (?,?,?,?,?)
                """, commandId, action, channel, mask, authorizationId);
    }

    public Map<String, Object> control(String commandId) {
        return jdbc.queryForList("""
                SELECT c.*, x.action, x.channel, x.mask, x.authorization_id AS control_authorization_id
                FROM device_command c
                JOIN countermeasure_4ch_command x ON x.command_id=c.command_id
                WHERE c.command_id=?
    ...
    public void addReceipt(String commandId, String commandNo, String resultCode, long now, String payload) {
        ...
        jdbc.update("""
                ...
                VALUES (?,?,?,?,?,?,?,?)
                """, UUID.randomUUID().toString(), commandId, inboxId, "PROTOCOL_4CH", resultCode, now, now, payload);
    }
    ...
    public void addOutbox(String id, String commandId, long now) {
        jdbc.update("INSERT INTO outbox_event(outbox_id,topic,payload,created_at,available_at,attempt_count) VALUES (?,?,?,?,?,0)",
                id, "device.control.countermeasure", commandId, now, now);
    }
}
