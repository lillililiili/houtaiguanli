...
class UavDepartureObservationTest {
 ...
 final UavAdvisoryService service=new UavAdvisoryService(events,null,access,null,null,clock,null,sms,voice,watch,null,null);
 ...
 @Test void waitsFullTenSeconds(){delivered();when(clock.nowMillis()).thenReturn(19999L);var r=service.observation("event");assertThat(r.status()).isEqualTo("WATCHING");assertThat(r.deadlineAt()).isEqualTo(20000L);verifyNoInteractions(watch);}
