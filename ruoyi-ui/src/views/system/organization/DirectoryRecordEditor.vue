...
const props = defineProps({ visible: Boolean, kind: { type: String, required: true }, row: { type: Object, default: null }, organization: { type: Object, default: null }, canEdit: Boolean })
...
const title = computed(() => `${props.row ? '编辑' : '新增'}${({ profile: '单位档案', contact: '联系人', binding: '来源映射' })[props.kind]}`)
watch(() => [props.visible, props.kind, props.row], () => {
  if (!props.visible) return
  const row = props.row || {}
  Object.keys(form).forEach(key => delete form[key])
  Object.assign(form, {
    name: row.name || '', org_code: row.org_code || '', parent_id: row.parent_id || '',
...
async function save() {
  ...
  try {
    ...
    const result = await directoryApi[method](id, body)
    emit('saved', result)
...
<template>
  <el-dialog :model-value="visible" :title="title" width="min(760px, calc(100vw - 32px))" :close-on-click-modal="!busy" :close-on-press-escape="!busy" :show-close="!busy" class="directory-record-dialog" @update:model-value="value => !value && close()">
    <el-alert v-if="!canEdit" title="当前账号只能查看资料，保存需要单位档案维护权限。" type="info" :closable="false" />
    ...
    <el-form label-position="top" :disabled="!canEdit || busy || blocked" @submit.prevent="save">
      <div class="form-grid">
        <template v-if="kind === 'profile'">
          ...
          <el-form-item label="单位类型" required><el-select v-model="form.organization_type"><el-option v-for="type in organizationTypes" :key="type.value" :label="type.label" :value="type.value" /></el-select></el-form-item>
          <el-form-item label="单位档案编码"><el-input v-model="form.org_code" :disabled="Boolean(row)" placeholder="新建时可留空，由系统生成" /></el-form-item>
          <el-form-item label="上级单位"><DirectorySelect v-model="form.parent_id" kind="organizations" :current-label="row?.parent_name || ''" /></el-form-item>
          <el-form-item label="统一社会信用代码或统一标识"><el-input v-model="form.credit_code" /></el-form-item>
          <el-form-item label="单位地址"><el-input v-model="form.address" type="textarea" autosize /></el-form-item>
          <el-form-item class="wide" label="业务职责"><el-input v-model="form.responsibilities" type="textarea" :rows="3" /></el-form-item>
          <el-form-item class="wide" label="档案说明"><el-input v-model="form.remarks" type="textarea" :rows="2" /></el-form-item>
          <p class="form-note wide">档案复用现有单位标识。建档不会创建账号或增加数据权限；单位不提供停用操作。</p>
        </template>
