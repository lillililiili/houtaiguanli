...
class NotifyFlowTest {
    @Test void verifiedAlarmWalksSmsWatchCallWatchThenCounter() {
        ...
        assertThat(NotifyFlow.phase("CONFIRMED", "SIMULATED_DELIVERED", "已送达", sms, "WAITING", null, null, sms + 2_000L, null, null)).isEqualTo(Phase.WATCHING);
        assertThat(NotifyFlow.phase("CONFIRMED", "SIMULATED_DELIVERED", "已送达", sms, "CALLING", null, null, sms + 3_000L, Presence.STILL_PRESENT, null)).isEqualTo(Phase.AUTO_CALL);
        ...
        assertThat(NotifyFlow.phase("CONFIRMED", "SIMULATED_DELIVERED", "已送达", sms, "BLOCKED", "最新位置已离开短信发出时所处的告警空域，不拨打电话", null, sms + 3_000L, Presence.LEFT, null)).isNull();
        assertThat(NotifyFlow.phase("CONFIRMED", "SIMULATED_DELIVERED", "已送达", sms, "BLOCKED", "短信发出后没有新的位置，或无法判断是否仍在告警空域，不拨打电话，也不记为已撤离", null, sms + 3_000L, Presence.UNKNOWN, null)).isEqualTo(Phase.AWAIT_COUNTER);
        assertThat(NotifyFlow.phase("CONFIRMED", "SIMULATED_DELIVERED", "已送达", sms, "UNAVAILABLE", null, null, sms + 3_000L, Presence.STILL_PRESENT, null)).isEqualTo(Phase.AWAIT_COUNTER);
        ...
        assertThat(NotifyFlow.phase("CONFIRMED", "SIMULATED_DELIVERED", "已送达", sms, "UNAVAILABLE", "未配置录音", null, sms + 3_000L, null, null)).isEqualTo(Phase.AWAIT_COUNTER);
