...
class AutoVoicePostgresTest extends AutoVoiceApiTest {
    ...
    @Test void upgradeFromPreviousVersionAndRepeatValidation() {
        String schema="auto_voice_upgrade_"+UUID.randomUUID().toString().replace("-","");var jdbc=new JdbcTemplate(root());jdbc.execute("create schema "+schema);
        try {
            Flyway.configure().dataSource(root()).schemas(schema).defaultSchema(schema).createSchemas(false).cleanDisabled(true).locations("classpath:db/migration","classpath:db/postgresql").target("202609160001").load().migrate();
